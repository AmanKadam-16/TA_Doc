...
//Intialise Login With Values
  Future<AuthLogin> InItAuthLogin()
  async {
    ...
    try
    {
     ...
     /* if()
      ...
      if(Platform.isAndroid|| Platform.isIOS) {
        ...
        final strFileData = await FileInteraction().getDataFromFile(
            FileInteraction.MTAFileName);
        if (strFileData.toString().trim().length == 0) // file is not exists
          ...
          objLoginInfo = AuthLogin.fromJson(valueMap);
            ...
            if (objLoginInfo.LoginID.length == 0) {
              objLoginInfo.CanGoForBiometricLogin = false; // means go for Login Screen
            }
  ...
  Future<String>  InItAuthEncryptedString(AuthLogin objLoginInfo)
  async {
...
// Clear loginInformation In file too (except GUID)
  Future<bool> ClearLoginInformationForLogOut()
  async {
    bool bResult=true;
    try    {
  ...
  Future<AuthLogin> LoginInformationForFirstLogin(String strCompanyCode,String strLoginID, String strPassword)
  async {
    AuthLogin objLoginInfo=new AuthLogin(""); //GUID will not change
    bool bResult=true;

    try    {
      objLoginInfo= await InItAuthLogin(); // to retreive old values along with GUID
  ...
  Future<AuthLogin> LoginInformationForFirstLoginViaApp()
  async {
    AuthLogin objLoginInfo=new AuthLogin(""); //GUID will not change
    bool bResult=true;
    try    {
  ...
  Future<AuthLogin> UpdateSuccessFullLoginInformation(String strCompanyCode, String strLoginID, String strPassword,LoginMode objLoginMode)
    async {
...
/// saving data in file
  Future<bool> SaveLoginInformationInTheFile(AuthLogin objLoginInfo)
  async {
    bool bResult=true;
    try
    {
      ...
      if(Platform.isAndroid|| Platform.isIOS)
      {
        ...
        await FileInteraction().saveDataInFile(
            strJsonData, FileInteraction.MTAFileName);
      }
    }
    on  Exception catch(e)
    {
