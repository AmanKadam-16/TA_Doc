...
class UserLogin{
    ...
    set UserID(iUserID)  // its identity column so only for Update and delete it will be use
    {
        _UserID=iUserID;
    }
    ...
    String get DepartmentIDs => _DepartmentIDs; // list of Department IDs separated by ',
    ...
    //todo: Internal Encryption decryption needed- not implemented
    String _LocationIDs='';
    String get LocationIDs => _LocationIDs; // list of Location IDs separated by ',
    ...
    set CanAdd(bCanAdd) /// save update non mandatory
    {
        _bCanAdd=bCanAdd;
    }
    bool _bCanEdit=false;
    bool get CanEdit => _bCanEdit;
    set CanEdit(bCanEdit) /// save update non mandatory
    {
        _bCanEdit=bCanEdit;
    }
    bool _bCanDelete=false;
    bool get CanDelete => _bCanDelete;
    set CanDelete(bCanDelete) /// save update non mandatory
    {
        _bCanDelete=bCanDelete;
    }
    bool _bCanView=false;
    bool get CanView => _bCanView;
    set CanView(bCanView) /// save update non mandatory
    {
        _bCanView=bCanView;
    }
    bool _bCanApproveOrRejectManualAttendance=false;
    bool get CanApproveOrRejectManualAttendance => _bCanApproveOrRejectManualAttendance;
    set CanApproveOrRejectManualAttendance(bApproveOrRejectManualAttendance) /// save update non mandatory
    {
    ...
    set CanApproveOrRejectOutDoorDuty(bApproveOrRejectOutDoorDuty) /// save update non mandatory
    {
    ...
    set CanApproveOrRejectLeave(bApproveOrRejectLeave) /// save update non mandatory
    {
    ...
    set CanAssignedLeavePolicy(bAssignLeavePolicy) /// save update non mandatory
    {
    ...
    set CanApproveOrRejectTour(bApproveOrRejectTour) /// save update non mandatory
    {
    ...
    set CanApproveMobileToUse(bCanApproveMobileToUse) /// save update non mandatory
    {
    ...
    set CanDefineGeoFencingForOffice(bCanDefineGeoFencingForOffice) /// save update non mandatory
    {
    ...
    set CanChangeEmployeeLoginDetails(bCanChangeEmployeeLoginDetails) /// save update non mandatory
    {
        _bCanChangeEmployeeLoginDetails=bCanChangeEmployeeLoginDetails;
    }

    bool _bCanUpdateEmployeeInventoryDetails=false;
    bool get CanUpdateEmployeeInventoryDetails => _bCanUpdateEmployeeInventoryDetails;
    set CanUpdateEmployeeInventoryDetails(bCanUpdateEmployeeInventoryDetails) /// save update non mandatory
    {
    ...
    set Reset(bResetPassword) // // During Login if this is true, compulsory send for new password form , no by pass
    {
    ...
    set  MarkedByUserID(String strMarkedByUserID )    // display Only , it will add in rest api
    {
    ...
    set  MarkedByUserLoginName(String strMarkedByUserLoginName )    // display Only , it will add in rest api
    {
    ...
    UserLogin.fromJson(dynamic json) {
         ...
         {
             _lstDepartment = (json['ListOfDepartment'] as List).map((i) => Department.fromJson(i)).toList();
         }
         else
         {
             _lstDepartment = [];
         ...
         {
             _lstLocation= (json['ListOfLocation'] as List).map((i) => TALocation.fromJson(i)).toList();
         }
         else
         {
             _lstLocation = [];
         ...
         if(json['Type']=="Admin" || json['Type']==6) _UserType=UserType.Admin;
         if(json['Type']=="HR"|| json['Type']==7) _UserType=UserType.HR;
         if(json['Type']=="Company"|| json['Type']==1) _UserType=UserType.Company;
         if(json['Type']=="CompanyNDepartment"|| json['Type']==4) _UserType=UserType.CompanyNDepartment;
         if(json['Type']=="Department"|| json['Type']==2) _UserType=UserType.Department;
         if(json['Type']=="Location"|| json['Type']==5) _UserType=UserType.Location;
         if(json['Type']=="Employee"|| json['Type']==3) _UserType=UserType.Employee;
         if(json['Type']=="DeviceManagement"|| json['Type']==11) _UserType=UserType.DeviceManagement;
         ...
         _bCanApproveOrRejectManualAttendance=json['CanApproveOrRejectManualAttendance'];
         ...
         _bCanUpdateEmployeeInventoryDetails=json['CanUpdateEmployeeInventoryDetails'];
    ...
    {// not tested
        ...
        map['CanApproveOrRejectManualAttendance']=_bCanApproveOrRejectManualAttendance;
        ...
        map['CanUpdateEmployeeInventoryDetails']=_bCanUpdateEmployeeInventoryDetails;
